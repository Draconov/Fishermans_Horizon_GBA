...
def test_options_model_host_contract(tmp_path: Path):
    compiler = shutil.which("g++") or shutil.which("c++")
    if compiler is None:
        pytest.skip("host C++ compiler unavailable")
    executable = tmp_path / "options_model_test"
    result = subprocess.run([
        compiler, "-std=c++17", "-Wall", "-Wextra", "-pedantic", "-Iinclude",
        "tests/host/test_options_model.cpp", "src/options_model.cpp", "src/flow_model.cpp",
